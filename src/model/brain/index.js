...
import { imgToPixelArray, isInstanceOfSupportedElement } from './imageUtilities'
...
class DiyNeuralNetwork {
  constructor(options, cb) {
    ...
    this.convertTrainingDataToTensors = this.convertTrainingDataToTensors.bind(
      this
    )
    this.formatInputsForPrediction = this.formatInputsForPrediction.bind(this)
    this.formatInputsForPredictionAll = this.formatInputsForPredictionAll.bind(
      this
    )
    this.isOneHotEncodedOrNormalized = this.isOneHotEncodedOrNormalized.bind(
      this
    )
    // model prep
  ...
  prepareForTraining(_dataRaw = null) {
    ...
    const unnormalizedTrainingData = this.neuralNetworkData.applyOneHotEncodingsToDataRaw(
      dataRaw
    )
